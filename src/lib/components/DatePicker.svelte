...
{#if expanded}
  <div
    ...
    class="overflow-hidden rounded-2xl border border-stone-200 bg-white p-3 shadow-sm"
  >
    <div class="flex items-center justify-between gap-2 px-1">
      <button
        ...
        disabled={viewIdx <= 0}
        on:click={() => step(-1)}
        class="rounded-lg p-1.5 text-stone-500 transition hover:bg-stone-100 hover:text-stone-800 disabled:pointer-events-none disabled:opacity-30"
      >
        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6" /></svg>
      </button>
      <span class="text-sm font-semibold text-stone-800">{MONTHS[viewMonth - 1]} {viewYear}</span>
      <button
        type="button"
...
{:else}
  <button
    ...
    transition:slide={{ duration: 220, easing: cubicOut }}
    on:click={expand}
    class="flex w-full items-center justify-between gap-3 overflow-hidden rounded-2xl border border-stone-200 bg-white px-4 py-2.5 text-left shadow-sm transition hover:border-stone-400 md:py-3"
  >
    <span class="flex items-center gap-2.5 text-base text-stone-900">
      <svg class="h-5 w-5 shrink-0 text-red-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" />
      </svg>
      {#if selectedEntry}
        <span class="font-medium">{summary(selectedEntry)}</span>
        <span class="text-sm text-stone-500">· {selectedEntry.streets.length} Straßen</span>
      {:else}
        <span class="text-stone-500">Abholtag wählen</span>
      {/if}
    </span>
    <svg class="h-4 w-4 shrink-0 text-stone-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6" /></svg>
  </button>
{/if}
